...
import externalAxiosInstance from '../externalAxiosConfig'; // Import the external axios instance

const MasterAgreementList: React.FC = () => {
  ...
  const fetchMasterAgreements = async () => {
    const url = '/master-agreements/established-agreements'; // Relative to external baseURL
    setLoading(true);
    setError(null);
    try {
          ...
          {agreements.map((agreement) => (
            <Grid item xs={12} sm={6} md={4} key={agreement.agreementId}>
              <Card
                ...
                onClick={() => navigate(`/master-agreements/${agreement.agreementId}`, { state: { agreementId: agreement.agreementId } })}
              >
                <CardContent>
                  <Typography
                    variant="h6"
...
export default MasterAgreementList;
